...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        SettingsFactory.loadSettings(self,
                    Setter(recentFiles=None, _conv=QVariant.toStringList),
                    Setter(resize=QSize(600, 500), _conv=QVariant.toSize),
                    Setter(move=QPoint(0, 0), _conv=QVariant.toPoint),
                    Setter(restoreState=None, _conv=QVariant.toByteArray)
                    )
        #self.recentFiles = settings['RecentFiles'].toStringList()
        #settings = QSettings()
    ...
    def closeEvent(self, event):
        if self.okToContinue():
            ...
            SettingsFactory.saveSettings(self,
                                Setter(recentFiles=self.recentFiles),
                                Setter(LastFile=self.filename, _conv=QString),
                                Setter(resize=self.size()),
                                Setter(move=self.pos()),
                                Setter(restoreState=self.saveState())
                                )
    ...
    def loadInitialFile(self):
        #settings = QSettings()
        #lastFile = SettingsFactory.loadSetting(self,
        #                        LastFile=SimpleStringSettingGetter(self))
        #fname = unicode(lastFile.toString())
        #fname = unicode(settings.value("LastFile").toString())
        fname = unicode(SettingsFactory.getSettings(self,
                            Setter(LastFile=None, _conv=QVariant.toString)))
